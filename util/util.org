#+begin_export latex
! Package \ty{uti} provides auxiliary functions for the \ty{never}
! package.

Our outline of \ty{uti} has hooks for imports, variables, and
functions.  \bpa{uti}{pa:uti}
#+end_export
...
We also store the program name in the variable \ty{program}.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:uti}>>=
...
#+end_src
#+begin_export latex
The variable \ty{program} is a package variable we can use anytime we
need to refer to the name of the program.
#+end_export
...
#+end_src
#+begin_export latex
  The date and the version are injected at compile time, so we declare
