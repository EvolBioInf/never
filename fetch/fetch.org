...
working. But just for good measure, let's also query the other nine
services provided by the Neighbors server at the time of writing. We
can classify these services by query, which may consist of a single
taxon ID, multiple taxon IDs, a single name, or multiple accessions.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:fet}>>=
...
\ty{accessions}, \ty{children}, and \ty{parent}.
#+end_export
