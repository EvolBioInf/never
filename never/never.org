...
programs. At this point, the queries of the functions and programs are
just that. To convert them into the full URLs necessary for generating
the hyperlinks sketched in Figure~\ref{fig:idx}, we first construct
the base URL and then use it to set the query URLs.
#+end_export
#+begin_src go <<Set index page data, Pr. \ref{pr:nev}>>=
  p.Title = "Neighbors"
  p.Functions = functions
  p.Programs = programs
  //<<Construct base URL, Pr. \ref{pr:nev}>>
  //<<Set query URLs, Pr. \ref{pr:nev}>>
#+end_src
#+begin_export latex
...
#+end_src
#+begin_export latex
The base URL consists of the net protocol, the host, and the
port. Host and port are given, but the protocol is either http if we
are running locally, or https otherwise.
#+end_export
#+begin_src go <<Construct base URL, Pr. \ref{pr:nev}>>=
  protocol := "https"
  if host == "localhost" {
	  protocol = "http"
  }
...
#+end_src
#+begin_export latex
We iterate over the functions and the programs and set their query
URLs.
#+end_export
#+begin_src go <<Set query URLs, Pr. \ref{pr:nev}>>=
...
variable for holding the page data and returns a handler function that
calls the ordinary function passed with the reader, the writer, and
the new page data as arguments.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:nev}>>=
