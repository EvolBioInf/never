...
nodes, which are structs to hold a taxon ID and a parent ID.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
  type Node struct {
	  Taxid int `json:"taxid"`
...
itself. From the current taxon and its parent we construct a node,
which we store in our slice of output values.
#+end_export
