...
#+end_src
#+begin_export latex
We declare the fields \ty{Title}, \ty{Date}, \ty{Ngenomes},
\ty{Ntaxa}, and \ty{Services} in \ty{PageData}. The numbers of taxa
and genomes are nominally integers, but we'd like to insert commans
into them, so we declare these numbers as strings.
#+end_export
...
mode. If we are \emph{not} matching in exact mode, we replace blanks
in the name by wild cards and also bracket the name by wild cards.
#+end_export
...
each one, except for the root, whose parent is itself. From the
current taxon and its parent we construct a node, which we store in
our slice of output values.
#+end_export
...
most recent common ancestor, and print the output.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:nev}>>=
...
#+end_src
#+begin_export latex
In our implementation of \ty{level} we extract the accession, get its
...
#+end_src
#+begin_export latex
We get the slice of accessions from the HTTP request.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:nev}>>=
