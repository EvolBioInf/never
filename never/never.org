...
#+end_src
#+begin_export latex
The files for visualizing the taxonomy are located in directory
\ty{vitax}. We make them available by registering a file server to
that directory.
#+end_export
#+begin_src go <<Serve style files, Pr. \ref{pr:nev}>>=
...
#+end_src
#+begin_export latex
We list the NCBI as the source of our raw data.
#+end_export
#+begin_src html <<List data sources, Pr. \ref{pr:nev}>>=
  ...
  the <a href="https://www.ncbi.nlm.nih.gov/">NCBI</a> to build a
  custom database.
#+end_src
#+begin_export latex
...
\ty{accessions}, we get the taxon ID and the corresponding accessions,
...
#+end_src
#+begin_export latex
We get the taxon ID through a call to the function \ty{getTaxa} we
still need to write. If \ty{getTaxa} returns no taxa, we set the taxon
ID to the non-existent taxid zero.
#+end_export
...
#+end_src
#+begin_export latex
If the taxon has no name, it doesn't exist and we skip the rest of the
loop.
#+end_export
...
returns their scientific names. We store its output in the struct
...
#+end_src
#+begin_export latex
We construct the child from its taxon ID and its name.
#+end_export
...
nodes, which are structs to hold a taxon ID, its name, and the parent
ID.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
  type Node struct {
...
itself. From the current taxon, its parent, and its name we construct
...
#+end_src
#+begin_export latex
We get the node's name and again skip to the end of the loop if we
encounter an error.
#+end_export
#+begin_src go <<Get node name, Pr. \ref{pr:nev}>>=
  name, err = neidb.Name(taxon)
  util.Check(err)
  if err != nil {
	  continue
...
#+end_src
#+begin_export latex
\subsubsection{\ty{num\_genomes\_rec}}
...
parent, its name, its raw genome counts, and its recursive genome
counts.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
...
#+end_src
#+begin_export latex
We get the parent, the name, and the genome counts.
#+end_export
#+begin_src go <<Get information, Pr. \ref{pr:nev}>>=
  //<<Get parent, Pr. \ref{pr:nev}>>
  //<<Get name, Pr. \ref{pr:nev}>>
...
#+end_src
#+begin_export latex
We look up the taxon's name with error checking.
#+end_export
#+begin_src go <<Get name, Pr. \ref{pr:nev}>>=
  name, err := neidb.Name(taxon)
  util.Check(err)
#+end_src
#+begin_export latex
