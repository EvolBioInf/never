#+begin_export latex
  The program \ty{never} is a webserver that provides access a REST API
  to the facilities of the Neighbors package; hence the name
...
#+begin_export latex
  ...
  end and a back end. The front end consists of a set of HTML files seen
  by the user via a browser. The back end consists of a command-line
  program seen by the sysadmin who starts the server. The back end is
  simpler than the front end, so we start with it.
#+end_export
...
#+end_src
#+begin_export latex
  We open the database given by the user and exit if there was an error.
#+end_export
#+begin_src go <<Respond to \ty{-d}, Pr. \ref{pr:nev}>>=
  db := tdb.OpenTaxonomyDB(*flagD)
#+end_src
#+begin_export latex
  We import \ty{tdb} and \ty{log}.
#+end_export
#+begin_src go <<Imports, Pr. \ref{pr:nev}>>=
...
#+begin_export latex
  ...
  we emulate Neighbors programs and access \ty{tdb} functions.
#+end_export
...
#+begin_export latex
  ...
  ID, its parent's taxon ID, and its full scientific name. To store
  these three items, we declare the \ty{struct} \ty{TaxiOut}.
#+end_export
...
that wraps \ty{tdb.TaxonomyDB}, to which we will later add methods
that wrap various queries.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
  type MyDB struct {
	  db *tdb.TaxonomyDB
...
#+end_src
#+begin_export latex
To obtain the three fields of \ty{TaxiOut} for all matching taxa, we
add the method \ty{TaxiHandler} to \ty{MyDB}. \ty{TaxiHandler}
is of type \ty{http.Handler}, which has the signature
\begin{verbatim}
type HandlerFunc func(ResponseWriter, *Request)
\end{verbatim}
Inside \ty{TaxiHandler}, we extract the query from the HTTP request,
query the database, and write the result in JSON format to the HTTP
writer.
#+end_export
...
substring mode. If we are matching in substring mode and the user
actually supplied a name, we replace blanks in the name by wild cards
and bracket the name by wild cards.
#+end_export
...
#+begin_export latex
  ...
  taxon IDs. Then we iterate over the taxon IDs and for each of them
  construct and store the taxon output.
#+end_export
...
#+end_src
#+begin_export latex
  We convert the output to json format and write it to the response
  writer.
#+end_export
...
#+end_src
#+begin_export latex
All that remains to finally emulate the \ty{taxi} program, is to
create the database wrapper and to register its method
\ty{TaxiHandler} at the URL \ty{taxi}.
#+end_export
