...
its output in the struct \ty{Accession}, which wraps an
accession and an assembly level.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
...
name, its common name, its raw genome counts, and its recursive genome
counts.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
