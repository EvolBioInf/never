...
#+end_src
#+begin_export latex
The files for visualizing the NCBI taxonomy are located in directory
...
slice of type \ty{GenomeCount}, which holds one count and one level.
#+end_export
#+begin_src go <<Types, Pr. \ref{pr:nev}>>=
...
genome counts, and print the output.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:nev}>>=
...
#+end_src
#+begin_export latex
We get the genome counts across all levels and store them.
#+end_export
...
#+end_src
#+begin_export latex
The variable \ty{assemblyLevels} holds the four possible assembly
levels.
#+end_export
#+begin_src go <<Variables, Pr. \ref{pr:nev}>>=
