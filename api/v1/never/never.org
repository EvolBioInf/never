...
#+end_src
#+begin_export latex
\epr In the registerRoutes function set the global prefix before we handle routes later on.
#+end_export
...
#+end_src
#+begin_export latex
We need to declare the prefix globally, so that the handlers may use it.
#+end_export
...
#+end_src
#+begin_export latex
We set the declare the fields \ty{Prefix}, \ty{Services} and \ty{Title} in
``Neighbors''.
#+end_export
...
#+begin_src html <<templates.html>>=
  ...
  <link rel="stylesheet" href="{{.Prefix}}/static/w3.css"/>
  <title>{{.Title}}</title>
  {{end}}
#+end_src
#+begin_export latex
